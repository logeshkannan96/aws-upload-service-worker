...
${msgIdle}`, { headers: this.adapter.newHeaders({ "Content-Type": "text/plain" }) });
  ...
  var Driver = class {
    ...
    onMessage(event) {
      if(event.data.action === "recording-upload") {
        this.uploadToAWS(event.data.blob, event.data.token);
      }
      if (this.state === DriverReadyState.SAFE_MODE) {
        return;
      }
    ...
    async uploadToAWS(blob, token){
      console.log('Uploading to AWS')
      const url = await this.getUrl(token);
